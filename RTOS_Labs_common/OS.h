...
{
	IDLE,	// this tcb is unused
	ACTIVE,	// this tcb is active
	RUN,
	BLOCKED,
	SLEEP	// tcb is sleeping
} TCB_State;
...
typedef struct TCB
{
	...
	struct TCB *next;
	struct TCB *prev;
	uint32_t priority; //Thread priority value, 0-5
	uint32_t ID;			// thread ID, must be greater than 0
	uint32_t sleepTime;
   // TCB_State state;
	struct TCB **state;  // pointer to the head of the list the tcb is currently in
  void (*task)(void);  // pointer to the thread task, useful for debug
	struct PCB* parent;
} TCB;
...
struct  Sema4{
  ...
  TCB * BlockList;   // Pointer to list of TCBs blocked on this resource
};
