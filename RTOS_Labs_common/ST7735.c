...
//------------ST7735_Message------------
//If the message is longer than a single line, it will wrap to the 
// next line. The number is always displayed after the string. If 
// NULL is passed into the function in palace of a number, no number 
// is displayed.
...
void ST7735_Message(uint32_t  d, uint32_t  l, char *pt, int32_t value){
  // write this as part of Labs 1 and 2
	int yVal = (d * 8) + l;
	OS_bWait(&LCDFree);
	ST7735_SetCursor(0, yVal);
	//int length = strlen(pt);
	//int numSubstrings = length / 20 + (length % 20 != 0); // Calculate n//20
	ST7735_OutString(pt);
	ST7735_OutDec(value);
	OS_bSignal(&LCDFree);
	//char substring[21]; // Buffer for each substring (+1 for null terminator)
	//int i;
//	for (i = 0; i < numSubstrings; ++i) {
//			int offset = i * 20;
//			strncpy(substring, pt + offset, 20); // Copy a chunk of the original string
//			substring[20] = '\0'; // Ensure null-termination
//			ST7735_OutString(substring);
//			if(i == (numSubstrings - 1) && value != NULL){
//				ST7735_OutDec(value);
//			}
//			ST7735_SetCursor(0,yVal + i + 1);
//	}
}
