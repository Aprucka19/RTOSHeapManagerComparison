...
// Semaphore for input availability
// Enables non-blocking reads
Sema4Type UARTDataAvailable;
...
void UART_Init(void){
  ...
  OS_InitSemaphore(&UARTDataAvailable, 0); // initialize data input semaphore
}
...
void UART0_Handler(void) {
    ...
    if (newData) {
        OS_bSignal(&UARTDataAvailable); // Signal that new data is available
    }
}
